...
test("h1: 기록된 행동을 그대로 재생하면 최종 상태가 일치한다", () => {
  const state = replay("h1");
  assert.deepEqual(snapshot(state, ["id","x","y","hp"]), [{"id":"a1","x":2,"y":0,"hp":4},{"id":"a2","x":1,"y":2,"hp":29},{"id":"e1","x":3,"y":0,"hp":5}]);
  assert.equal(state.round, 6);
  assert.equal(state.winner, null);
  assert.equal(state.current, "e1");
  assert.deepEqual(turnOrder(state), ["e1","a1","a2"]);
});
